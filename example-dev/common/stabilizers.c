...
__attribute__((noinline)) int wrapper(input_t *config, input_t *iv, input_t *key,
                                      input_t *plaintext, uint8_t *output)
{
    return run_test(config, iv, key, plaintext, output);
}
                 ...
                 const void *new_stack)
{
    ...
    asm volatile(""
                 ...
                 : "=A"(result)
                 : "r"(new_stack + STACK_SIZE), "r"(config), "r"(iv), "r"(key), "r"(plaintext),
                   "r"(output), "p"(wrapper)
                 : "memory");
